...
```js
       function countDown(time) {
            ...
            const day = Math.floor(interval / (1000 * 60 * 60 * 24)).toString().padStart(2, '0')
            const t = Math.floor(interval / (1000 * 60 * 60) % 24).toString().padStart(2, '0')
            const m = Math.floor(interval / (1000 * 60) % 60).toString().padStart(2, '0')
            const s = Math.floor(interval / 1000 % 60).toString().padStart(2, '0')
...
function getAllSet() {
      ...
      console.log(getAllSet([12, 2, 4, 4, 5, 6, 's'], ['d', 'f', 'gs', 'f', 'f', 'd', 'a'], [2, 0, 02, 4, 2, 1, 1, '0', 8, 8]))
```
...
var intersection = function(nums1, nums2) {
  ...
  for(let i=0;i<minLengthArr.length;i++){
     if( maxLengthArr.includes(minLengthArr[i])&&!result.includes(minLengthArr[i])){
...
```js
      ...
      function add(a, b) {
        ...
        let f = 0 //"进位"
        let sum = ''
        for (let i = maxLength - 1; i >= 0; i--) {  //注意此处 是倒序循环遍历
...
class EventMap {
    ...
    off(eventName, handler) {
        if (this.eventMap[eventName]) {
            this.eventMap[eventName].splice(this.eventMap[eventName].indexOf(handler), 1)
